...
function reportPath(file){const relative=path.relative(root,file);return (relative==='..'||relative.startsWith('..'+path.sep)||path.isAbsolute(relative)?path.basename(file):relative).split(path.sep).join('/');}
