...
Lithography (Modules 07 and 08) leaves the wafer covered in a stencil: a few tens of nanometres of photoresist with holes and lines in it. The transistor is made of silicon (the channel), silicon-germanium (SiGe, a strained channel and a sacrificial layer), silicon nitride (spacers, etch stops and masks), silicon dioxide (the general insulator), hafnium oxide (the gate insulator), titanium nitride (the gate metal), tungsten (contact plugs) and copper (wires). Something has to cut the resist's pattern into those materials, in the right places, to the right depth, with vertical walls, without damaging what is underneath, a few hundred times per wafer. That something is **etch**: the controlled removal of material through the openings in a mask. An etch is **isotropic** if it removes material at the same rate in every direction and **anisotropic** if it removes material straight down only.
...
> **Worked example: why wet etch cannot pattern a gate.** If a film of thickness t is etched isotropically through a mask opening of width w, the etch proceeds sideways under the mask by t on each side, so the finished feature is w + 2t wide with rounded sidewalls. For a 20 nm gate line in a 20 nm poly film: 20 + 2 × 20 = 60 nm, an undercut of 100% of the line, which no bath chemistry can fix. For a 5 nm spacer film beside a 30 nm feature the undercut is 5 nm per side, a sixth of the feature and tolerable, which is why some spacer-level and liner recesses are still done wet. The rule: an isotropic etch cannot resolve features smaller than roughly twice the film thickness.
...
> **Intuition:** The radical is the fuel, the ion is the match, and the passivant (next section) is a fireproof coat on the sides. Fuel alone smoulders (5 Å/min); a match alone scorches a spot (2 Å/min); fuel plus match burns fast, but only where the match lands, which is straight down on the floor of the feature.
...
> **Intuition: paint one coat, wipe one coat.** Imagine a paint that sticks only to bare wood and never to itself, and a cloth that removes only paint and never wood. However long you paint, you get one coat; however hard you wipe, you remove that coat and no more. The total is counted in cycles, not seconds.
...
| Hot H3PO4 nitride etch | 85% acid at ~160 °C; Si3N4 ~4–6 nm/min; SiO2 0.1–0.5 nm/min | The only selective nitride strip; enables the NAND nitride pull |
...
| Isotropic undercut rule | Finished width = w + 2t; features must be ≥ ~2x film thickness | Why wet etch stopped patterning |
...
| Cryogenic channel-hole etch | −40 to −70 °C; TEL: 10 µm in 33 min (~2.5x conventional, 84% lower GWP); Lam Cryo 3.0: 2.5x rate, <0.1% top-to-bottom CD deviation | Roughly halves the chamber count |
...
- **"Copper interconnects are etched."** → Copper has no volatile etch product at practical temperatures, which is why the BEOL switched to damascene (etch the dielectric, fill with copper, polish) in 1997; aluminium and tungsten are etched, copper, cobalt and ruthenium lines are formed by fill.
