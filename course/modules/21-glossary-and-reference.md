...
This is a reference volume, so a reader who has skipped everything should still be able to use its tables. These eight facts are enough:
...
- NMOS transistors switch with electrons, PMOS with holes; CMOS logic pairs one of each so that a logic gate draws current only while it is switching (Module 11).
...
- Yield is the fraction of good output; die yield falls exponentially with die area times defect density (Module 13).
...
Terms are alphabetized by the acronym or the first word of the term as it is normally written; where an acronym is the common usage, the expansion follows in parentheses. Four rules apply to every entry. Every symbol in a formula is named with its unit in the same sentence. Every material is given its plain name at first mention (tantalum nitride before TaN). Every number is followed by a clause saying what it means. Every acronym inside a definition is either expanded or points to its own entry with "see X". Five worked-example boxes sit directly under the formula entries they belong to (Arrhenius, Cpk, EOT, Negative binomial, Subthreshold swing), and one intuition box sits at Plasma, because the picture it gives explains half of the etch vocabulary at once.

- **1T1C** — The DRAM cell (see DRAM): one access transistor (1T) that acts as a switch and one storage capacitor (1C) that holds the bit as charge. The capacitor holds ~10–20 fF (femtofarads, 10⁻¹⁵ F); at ~15 fF and 1 V that is a charge Q = C × V ≈ 1.5 × 10⁻¹⁴ coulombs, only about 100,000 electrons, which leak away through the access transistor in tens of milliseconds. That is why every cell must be refreshed every ~64 ms, and why the capacitor is built as a tall pillar with an aspect ratio above 50:1 (see Aspect ratio): the cell footprint is tiny but the capacitor still needs enough surface area to hold those electrons. (Module 15)
...
- **ALD (Atomic Layer Deposition)** — Film growth by alternating, self-limiting half-reactions: a precursor pulse (see Precursor), a purge, a reactant pulse, a purge. "Self-limiting" means the surface can hold only one layer of adsorbed precursor molecules, so growth stops by itself until the next reactant is pulsed, no matter how long the pulse lasts. Each cycle deposits ~0.1 nm, about half an atomic layer, so a 2 nm film takes ~20 cycles, and the film is equally thick on every surface the gas can reach, including the bottom of a deep trench (see Conformality). Used for high-k gate dielectrics, liners, and spacers. (Module 06)
...
> **Worked example:** Take Ea = 0.7 eV, a typical value for the defects behind infant-mortality failures, T_use = 55 °C = 328 K and T_stress = 125 °C = 398 K. Then Ea/k = 0.7 / (8.617 × 10⁻⁵) ≈ 8,120 K, and 1/328 − 1/398 = 0.003049 − 0.002512 = 0.000537 K⁻¹, so the exponent is 8,120 × 0.000537 ≈ 4.36 and AF = e^4.36 ≈ 78. Ten hours of burn-in at 125 °C therefore exposes the same weak parts that ~780 hours, about a month, of normal use at 55 °C would. That is the whole economic case for burn-in: a month of early field failures is pulled forward into a ten-hour oven step.
...
- **Foundry** — A company that manufactures wafers to other companies' designs and sells no chips of its own (TSMC, Samsung Foundry, Intel Foundry, SMIC, GlobalFoundries, UMC, Rapidus). Contrast Fabless and IDM. (Module 20)
...
- **High-NA EUV** — 0.55 NA EUV (ASML EXE:5000/5200), ~8 nm resolution. The optics are anamorphic: they demagnify 4× in one direction and 8× in the other, so that the 6-inch mask can be kept while the larger mirrors capture the wider cone of light; the price is a half-size exposure field (26 × 16.5 mm), so large dies need two stitched exposures. (Module 08)
...
- **InFO (Integrated Fan-Out)** — TSMC's fan-out wafer-level package (see Fan-out) with RDL built directly over molded dies; used in Apple processors. (Module 17)
...
- **NVLink / NVSwitch / NV-HBI** — NVIDIA's GPU-to-GPU interconnect, the switch chip that fabrics it into an all-to-all network, and the 10 TB/s die-to-die link joining Blackwell's two dies. (Module 19)
...
- **RC delay** — Interconnect delay proportional to resistance × capacitance of a wire; it now dominates over transistor delay in most paths. Shrinking hurts both factors: a narrower wire has higher resistance (and copper's resistivity itself rises when the wire is thinner than the distance electrons travel between collisions), while closer wires have higher capacitance to their neighbours. Hence low-k dielectrics, airgaps, and thinner barriers (see Low-k dielectric, Airgap, Barrier layer). (Module 12)
...
- **Reflow** — Heating an assembly above the solder's melting point so that the joints form; ~220–250 °C for SAC305, the standard lead-free solder of tin with 3% silver and 0.5% copper, which melts at 217 °C. The whole package expands and contracts through that swing, which is where warpage comes from (see Warpage, CTE). (Module 16)
...
- **SoIC (System on Integrated Chips)** — TSMC's hybrid-bonded 3D die stacking (see Hybrid bonding), used in AMD 3D V-Cache and MI300. (Module 17)
...
- **Threshold voltage (Vt)** — Gate voltage at which the channel turns on; ~0.2–0.4 V, set by the work-function metal choice (see Work function, Work-function metal); PDKs offer 4–6 flavors so that designers can trade leakage for speed cell by cell. (Module 11)
...
- **Yield** — Fraction of good output. **Line yield**: wafers surviving the flow without being scrapped. **Die yield**: good dies per wafer, which falls exponentially with die area times defect density (see Poisson yield model, Murphy yield model, Negative binomial yield model). **Assembly/test yield**: good packages. **Compound yield** is their product, worked through in the Master Process Flow. (Module 13)
...
> **Intuition:** Read the table as a ladder. Each rung is 3–10× smaller than the one above, and the whole ladder is ten orders of magnitude, roughly the ratio of the Earth's diameter to a golf ball. Everything below the human-hair rung is invisible to the eye, and every transistor rung (gate length, sheet thickness, EOT) sits below the 13.5 nm EUV wavelength, which is why the lithography entries in the Glossary talk about k1 and NA rather than wavelength: the scanner prints things smaller than its own light, and the tricks that let it do so are what those two symbols measure. The Notes column says why each size is what it is.
...
| EUV wavelength | 13.5 nm | Tin plasma emission; the only wavelength at which a good multilayer mirror could be built | 08 |
...
5. Static timing analysis, IR-drop (supply-voltage droop across the on-chip power grid) and EM (electromigration) analysis, timing closure iterations. (19)
...
33. Optional epitaxial layer deposition (~2–5 µm p/p+ or p/p− epi), giving a defect-free device layer over a bulk that can getter metals. (03)
...
1. **Clean**, because a single particle under the next film prints as a defect in every layer above it.
2. **Anti-reflective coating**, because light reflecting off the layer below would interfere with the incoming light and print standing-wave ripples into the resist sidewalls.
3. **Hard mask** (silicon nitride, titanium nitride, or carbon), because the resist is too thin and too soft to survive a deep etch; the pattern is first transferred into a tougher film that does the masking.
4. **Resist coat** on the track, spun to a uniform film tens of nanometres thick.
5. **Expose** (DUV or EUV) on the scanner, which projects the mask pattern as an aerial image into the resist.
6. **Post-exposure bake (PEB)**, because in a chemically amplified resist exposure only creates acid; the bake drives the acid's deprotection chemistry that turns exposure into solubility.
7. **Develop**, dissolving the exposed (positive) or unexposed (negative) resist to leave the stencil.
8. **CD/overlay metrology** before etch, because a bad resist pattern can be stripped and reworked for the cost of a coat and an exposure, whereas a bad etch scraps the wafer.
9. **Etch**, transferring the stencil into the hard mask and then the layer beneath.
10. **Strip** the resist and hard mask residue, then **clean**, because polymer left on the sidewalls would be buried by the next deposition.
11. **Inspection** after etch, because particles and bridges are easiest to find while the layer is still the top surface and hardest to find once it is covered.
...
Each metal level Mx (x = 1 to ~15–18) is a dual-damascene loop: [low-k dielectric deposition (PECVD SiOCH) → etch stop → hard mask (TiN) → via litho/etch → trench litho/etch → strip/clean → TaN barrier (PVD/ALD) → Co or Ru liner → Cu seed (PVD) → Cu electroplating with superfill → anneal → Cu CMP → cap (SiCN)]. The via and trench are etched first and filled together because copper cannot be etched (see Damascene); the barrier stops copper diffusing into the dielectric; the liner makes the copper wet the wall; the cap seals the copper top. Written once for the tight pitch levels and once for the upper levels.
...
> **What can go wrong:** One bad die scraps a 12-high stack, and it scraps it after the stack has consumed twelve good dies' worth of TSV, thinning and bonding work. The stack's yield is the product of thirteen die yields (twelve DRAM plus the base), so even at 98% per die only ~77% of stacks would be good before any bonding loss (see the compound-yield worked example below). That arithmetic is why every DRAM die is probed for KGD at step 82 before it is ever stacked, why the probe includes a burn-in-like stress, and why bonding-induced failures (a misaligned microbump, a crack from the TCB force) are screened again at step 86 before the stack ships.
...
> **Worked example:** Compound yield. Multiply the yields of the stages a die passes through. Line yield (wafers that survive the flow) ~0.95. Die yield ~0.6, roughly what the yield models give for an 800 mm² die at a D0 between the mature ~0.05 and ~0.1/cm² (see the Negative binomial worked example in the Glossary). Assembly yield through CoWoS ~0.98. Final test and burn-in yield ~0.97. The product is 0.95 × 0.6 × 0.98 × 0.97 ≈ 0.54: roughly half of the GPU dies started become sellable packages, and the die-yield term dominates, which is why D0 gets more engineering attention than any other number in the fab. For the HBM branch the arithmetic is harsher because it multiplies per die: a 12-high stack plus base die is 13 dies, and at 98% KGD accuracy per die the stack yield before any bonding loss is 0.98¹³ ≈ 0.77. Raise KGD accuracy to 99% and the same stack yields 0.99¹³ ≈ 0.88. No process step in Stage K moves stack yield as much as that one test, which is why known-good-die testing is the gating technology of HBM.

### Stage O: Module, board, server, rack (Module 19)

What comes in is finished GPU packages; what goes out is a running, monitored data center. The constraint shifts from nanometres to kilowatts: a rack draws ~120–130 kW and must be cooled by liquid, and a single failed GPU among 72 in one NVLink domain takes the whole domain out of a training job.
...
How to read it. Every arrow (→) reads "sells to", left to right, without exception; the one line that used to run backwards (the system builders and their suppliers) has been rewritten so that it reads forward too. Seven role words recur. A **foundry** makes wafers for others and sells no chips of its own. A **fabless** company designs chips and owns no fab. An **IDM** (integrated device manufacturer) designs and fabricates its own chips. An **OSAT** (outsourced semiconductor assembly and test house) packages and tests chips for hire. An **ODM** (original design manufacturer) designs and builds servers and racks that a brand or hyperscaler sells or uses. An **OEM** (original equipment manufacturer) sells systems under its own name. A **hyperscaler** runs data centers at the scale of hundreds of thousands of servers and is the end customer. Market shares are approximate and dated "as of 2025"; they move. Each sub-list ends with a note on its choke point, the relationship in it that has no ready substitute.
...
- Choke point: ASML is the only EUV tool maker, and its optics come only from Zeiss SMT and its drive lasers only from Trumpf; Hanmi and Besi are the only qualified sources of the TCB and hybrid bonders that HBM stacking and SoIC depend on.
...
- **Arm** (CPU IP) → NVIDIA (Grace, Vera), Apple, Qualcomm, AWS, Ampere. **Synopsys, Cadence, Alphawave, Rambus** (PHY/interface IP: HBM, PCIe, UCIe, SerDes; a SerDes is the serializer/deserializer circuit that drives a high-speed link, and a PHY is its physical-layer block) → NVIDIA and other fabless.
...
- **TSMC** (wafers on N4/N3/N2; CoWoS/SoIC/InFO packaging) → NVIDIA (~$20B+ per year by 2025), Apple (largest customer at ~20–25% of revenue, as of 2025), AMD, Broadcom, Qualcomm, MediaTek, Marvell, Intel (some products), Google/AWS/Microsoft custom ASICs via Broadcom/Marvell/Alchip/GUC.
...
- The chain spans ten orders of magnitude, from a 2 m rack to a 0.2 nm atom; every transistor dimension is below the 13.5 nm EUV wavelength, which is why lithography is about k1 and NA, not wavelength.
...
- EUV delivers ~20× fewer photons per unit dose than ArF (~14 vs ~290 per nm² at production doses), so shot noise, not optics, sets the floor on EUV feature size and dose.
...
|---|---|---|
| Wafer diameter / thickness / mass | 300 mm / 775 µm / ~127 g | Every tool, FOUP and recipe in the fab is built around this one disc; the 775 µm exists only for handling and is ground away in packaging |
...
| Reticle field | 26 × 33 mm = 858 mm² (0.33 NA); 26 × 16.5 mm (High-NA) | No monolithic die can exceed 858 mm², so Blackwell is two dies and High-NA halves the field again |
| ArF / EUV wavelength | 193 nm / 13.5 nm | NA 1.35 / 0.33 and 0.55; the wavelength is larger than every transistor dimension, so k1 and NA do the work |
...
| Photons per nm² at production dose | ~14 (EUV, 20 mJ/cm²) vs ~290 (ArF, 30 mJ/cm²) | ~20× fewer photons per dose is the root of EUV stochastic failures; EUV dose cannot be cut for throughput the way ArF dose could |
...
| Wafer price N7 / N5 / N3 / N2 | ~$10k / ~$16–17k / ~$18–20k / ~$30k | List-price estimates, ~2025; price now rises faster than density, so cost per transistor has stopped falling |
| Cost per good N2 die (~800 mm²) | ~$30k wafer × ~45% yield ≈ ~30 good dies ≈ ~$1,000 of silicon each | Silicon is a minority of a finished GPU's cost once HBM, CoWoS, test and margin are added; yield, not wafer price, is the lever |
...
| D0, mature vs ramp | ~0.05–0.1 vs ~0.5 /cm² | Killer defects; D0 is the single number that sets die yield and hence cost per good die |
| Poisson yield, 800 mm² die at D0 = 0.1 | e^(−8 × 0.1) ≈ 45% | Why big dies bin and repair, and why chiplets exist |
| EUV tool price / throughput | ~$200M (NXE:3800E, ~220 wph); ~$380M (EXE:5200) | Each EUV layer costs 2–3× a DUV layer in tool time, so EUV layer count drives wafer price |
...
| Blackwell package | 2 × ~800 mm² dies, 208 B transistors, 8 × HBM3E, CoWoS-L | The largest package in volume production; every number above meets in it |
...
| Burn-in acceleration (Arrhenius) | AF ≈ 20–100× at 125 °C vs 55 °C for Ea 0.7 eV | Hours in an oven stand in for months of field use; the range covers the spread of activation energies |
...
- "A 3 nm chip has 3 nm features" → Nothing on the chip is 3 nm. The gate is ~12–16 nm long, CPP is ~48 nm, the tightest metal pitch ~23 nm; the name is a model year.
...
- "The rack-to-atom chain is a few orders of magnitude" → It is ten, from a 2 m rack to a 0.2 nm atom, roughly the Earth's diameter to a golf ball.
