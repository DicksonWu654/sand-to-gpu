...
A **plasma etch** uses a gas containing electrons, ions and reactive species. Neutral reactive species support chemical removal, while ions can provide directed energy at the surface. Their interaction helps combine useful chemistry with directional action. The objective is not simply to blast material away as forcefully as possible: that would also damage or remove things that should remain.
...
**Doping** introduces selected atoms to change a semiconductor's electrical properties. In silicon, certain dopants make electrons the dominant mobile carriers; others make **holes**, the mobile absence of an electron in the valence-band system, dominant. These are called **n-type** and **p-type** material respectively.
...
Implantation also damages the crystal by displacing atoms. Furthermore, a dopant atom does not necessarily contribute the intended electrical behavior immediately after it stops. **Activation** places dopants into an electrically useful state. **Annealing**, a controlled heat treatment, can repair damage and improve activation, but can also cause atoms to move.
