...
A fluid can pass through distillation equipment. **Distillation** separates substances using differences in their tendency to evaporate and condense. One separation is rarely sufficient when the desired contamination level is extremely low. Repeated separation, careful chemistry and clean equipment work together to remove contaminants while retaining the silicon-containing feedstock.
...
An analogy is refining a programming-language library before assembling a complete application. You can make the input dependable without yet establishing the final structure in which it will be used. Chemical purification establishes a clean starting population of atoms. Crystal growth will establish how those atoms fit together over the entire wafer.
...
One useful technique is **gettering**, the deliberate collection of unwanted impurities in locations where they will do less harm. The analogy is putting a drain away from a work surface: the goal is to direct contamination away from the region where it would damage the product. This does not mean that more oxygen is always better. The amount, distribution and later heat treatments must suit the device being made.
