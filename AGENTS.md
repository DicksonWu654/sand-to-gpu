...
- Keep AI reading handoffs explicit and local: snapshot selected text before focus changes, preview exact clipboard contents, retain the passage when nearby context is disabled, and never put private excerpts in URLs or auto-submit them. Restore selection without scrolling, pause pending narration as well as active playback, and leave source text/extraction unchanged. Intercept external navigation and clipboard in automated QA.
